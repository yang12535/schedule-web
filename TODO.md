...
### 后端安全加固
- [ ] **为密码生成改用 `crypto.randomInt()`**
  - 位置：`server.js:11-13`
  - 当前使用 `Math.random()`，非加密安全

- [ ] **修复 X-Forwarded-For 速率限制绕过**
  - 位置：`server.js:58-65`
  - 直接信任客户端传入的 X-Forwarded-For，可伪造随机 IP 绕过限流

- [ ] **为 rateLimitStore 添加 TTL 清理**
  - 位置：`server.js:37-56`
  - 永不过期，可被恶意填充导致 OOM
...
- [ ] **修复 totalPeriods 与 periodSettings 长度不一致**
  - 位置：`server.js:232-254`
  - 两者间无任何一致性校验

### 前端 Bug 修复
- [ ] **修复 localStorage JSON 解析崩溃**
...
| #8 | 数据文件损坏导致服务崩溃 | ⚠️ 部分修复 | 备份后仍 `throw err` |
| #9 | 保存数据时目录不存在 | ✅ 已修复 | |
| #10 | 日志文件路径遍历漏洞 | ⚠️ 部分修复 | 存在 `startsWith` 前缀绕过 |
| #11 | 缺少全局错误处理中间件 | ✅ 已修复 | |
| #12 | 导出文件名编码问题 | ✅ 已修复 | |
